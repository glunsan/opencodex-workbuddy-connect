...
4. Run the copied `scripts/Install.ps1` with `-Region auto` unless a different region was chosen. The installer registers only the discovered region(s) with OpenCodex and may create its documented startup entry. Pass the documented state directory when one is already in use so an update retains local continuity.
