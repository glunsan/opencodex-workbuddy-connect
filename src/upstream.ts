...
export class WorkBuddyUpstreamClient {
  ...
  async fetchModels(credential: WorkBuddyCredential): Promise<readonly WorkBuddyUpstreamModel[]> {
    // WorkBuddy AI 5.5.2 uses /v2 here; CN still uses /console.
    const modelPath = regionOf(credential.domain) === 'global'
      ? '/v2/enterprises/personal/models'
      : '/console/enterprises/personal/models'
    const response = await fetch(`${chatBase(credential)}${modelPath}`, {
      headers: {
        'Authorization': `Bearer ${credential.accessToken}`,
        'Accept': 'application/json',
        'Origin': originReferer(credential),
